...
public class Stage1 extends BasicGameState {
    ...
    public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
        ...
        asteroid_img = new Image("img/asteroid2.png");
        background_img = new Image("img/Earth.png");
        ...
        in= gc.getInput();
        VARS.player = new PlayerObject(player_img,100,256,0, player_img.getWidth(), player_img.getHeight());

        File f = new File("Highscore.txt");
        try {
            PrintWriter pw = new PrintWriter(f);
            pw.write("Player1: "+VARS.score);
    ...
    public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
        ...
        background_img.draw();
        g.drawString("Score:"+VARS.score,10,20 );

        VARS.player.getImg().draw(VARS.player.getX(),VARS.player.getY());
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
        ...
        for(SpikeObject spike : VARS.spike_list){
            spike.update();
            if(VARS.player.getHitbox().intersects(spike.getHitbox())){
                VARS.game_over=true;
            }
            for(GameObject shot: VARS.shot_list)
        ...
        for(AsteroidObject asteroid : VARS.asteroid_list){
            ...
            if(VARS.player.getHitbox().intersects(asteroid.getHitbox())){
                VARS.game_over=true;
            }
            for(GameObject shot:VARS.shot_list)
